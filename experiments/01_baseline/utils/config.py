...
def load_config(exp_config: str, exp_name: str) -> dict:
    """Load config: deep-merge a hardcoded base.yaml with the given --config file.

    Two-way merge only (base.yaml < exp_config; exp_config wins on conflicting
    keys). ``exp_name`` is not used to look up a third config file — it is
    stored under cfg["exp_name"] as a label (used for run naming/logging) if
    the loaded config doesn't already define one.
    """
